...
class TurnIntoCustom extends CommonVar {
            ...
            if (select) {
              ...
              if (select.children.length > 0) {
                for (let i = 0; i < select.children.length; i++) {
                  lists += `<li class="${this.optionClass} ${
                    select.children[i].selected ? "selected" : ""
                  }">${select.children[i].innerHTML}</li>`;
                }
              }
  ...
  declaration(wrapper, option) {
    ...
    this.observe =
      option && option.observe == false && this.search == false ? false : true;
    this.reset =
      option && option.reset ? option.reset : this.search ? this.search : false;
    this.resetIcon =
      ...
      if (this.options) {
        ...
        if (this.searchType == "similar") {
          ...
          lists = this.options.filter(
            (data) => data.title.toLowerCase() === value.toLowerCase()
          );
        } else {
          lists = this.options.filter((data) =>
        ...
        if (select) {
          ...
          if (this.options && this.options.length > 0) {
            ...
            for (let i = 0; i < this.options.length; i++) {
              lists += `<li data-value="${this.options[i].value}" data-index="${
            ...
            if (hasSelected) {
              if (this.multiple) {
                ...
                selectedLi = [
                  listUl.children[selectedIndex[selectedIndex.length - 1]],
                ];
              }
            }
            ...
            if (this.search) {
              ...
              inp.oninput = () =>
                this.doSearch({ customSelect, inp, value: inp.value });
