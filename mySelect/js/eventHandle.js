...
class EventHandle extends CommonVar {
    ...
    if (
      ...
      if (
        (this.prevCustom &&
          ...
          !e.target.closest(`.${this.wrapperClass}`)?.querySelector("select")
            ?.multiple) ||
  ...
  keyPress(e) {
    if (this.prevCustom) {
      let crrLi = this.prevCustom.querySelector(
        ...
        ? this.prevCustom.querySelector(`.${this.dropDownDivClass} .active`)
        : this.prevCustom.querySelector(`.${this.dropDownDivClass} .selected`);
  ...
  triggerObserver(select) {
    try {
      ...
      if (targetNode) {
        targetNode.setAttribute("data-mutation", new Date().getTime());
      }
    } catch (error) {
